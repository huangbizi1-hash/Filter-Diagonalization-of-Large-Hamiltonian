...
def build_julia_batch_script(terms_cos, terms_sin):
    ...
    * Stdout line (last): JSON with warmup_s, eval_s, n_eval_waves
    ...
    L += ['function main()',
          ...
          '',
          '    # Per-thread buffers to avoid data races in threaded wave loop',
          '    out    = Vector{Float64}(undef, N * n_waves)',
          '    n_th   = Threads.nthreads()',
          '    phases = [Vector{Float64}(undef, N) for _ in 1:n_th]',
          '    cos_ps = [Vector{Float64}(undef, N) for _ in 1:n_th]',
          '    sin_ps = [Vector{Float64}(undef, N) for _ in 1:n_th]',
          '',
          ...
          '        exp_cos, exp_sin, phases[1], cos_ps[1], sin_ps[1])',
          '',
          '    # Timed eval: parallel over waves (each writes to disjoint out slice)',
          '    t_eval = @elapsed Threads.@threads for iw in 1:n_waves-1',
          '        tid = Threads.threadid()',
          '        ofs = iw * N + 1',
          '        kid = iw * 4 + 1',
          '        eval_one_wave!(@view(out[ofs:ofs+N-1]), X, Y, Z,',
          '                       kb[kid], kb[kid+1], kb[kid+2], kb[kid+3],',
          '                       exp_cos, exp_sin, phases[tid], cos_ps[tid], sin_ps[tid])',
          '    end',
          '',
...
def build_julia_hn_cse_script(expr_cos, expr_sin):
    ...
    L += ['function main()',
          ...
          '    t_warmup = @elapsed eval_one_wave!(',
          '        @view(out[1:N]), X, Y, Z, kb[1], kb[2], kb[3], kb[4],',
          '        phases[1], cos_ps[1], sin_ps[1])',
          '    t_eval = @elapsed Threads.@threads for iw in 1:n_waves-1',
          '        tid = Threads.threadid()',
          '        ofs = iw * N + 1; kid = iw * 4 + 1',
          '        eval_one_wave!(@view(out[ofs:ofs+N-1]), X, Y, Z,',
          '                       kb[kid], kb[kid+1], kb[kid+2], kb[kid+3],',
          '                       phases[tid], cos_ps[tid], sin_ps[tid])',
          '    end',
...
def build_julia_combined_from_snippets(
    ...
    L += ['function main()',
          ...
          '    t_warmup = @elapsed eval_one_wave!(',
          '        @view(out[1:N]), X, Y, Z,',
          '        kb[1], kb[2], kb[3], kb[4], c, phases[1], cos_ps[1], sin_ps[1])',
          '',
          '    t_eval = @elapsed Threads.@threads for iw in 1:n_waves-1',
          '        tid = Threads.threadid()',
          '        ofs = iw*N + 1',
          ...
          '        eval_one_wave!(@view(out[ofs:ofs+N-1]), X, Y, Z,',
          '                       kb[kid], kb[kid+1], kb[kid+2], kb[kid+3],',
          '                       c, phases[tid], cos_ps[tid], sin_ps[tid])',
          '    end',
          '',
          '    open(out_file,"w") do io; write(io, out); end',
          '',
...
def build_julia_filter_from_snippets(snippets: list, m: int,
    ...
    L += ['function main()',
          ...
          '    t_warmup = @elapsed eval_one_wave!(',
          '        @view(out[1:N]), X, Y, Z,',
          '        kb[1], kb[2], kb[3], kb[4], phases[1], cos_ps[1], sin_ps[1])',
          '',
          '    t_eval = @elapsed Threads.@threads for iw in 1:n_waves-1',
          '        tid = Threads.threadid()',
          '        ofs = iw*N + 1',
...
def build_julia_cse_filter_script(m: int, coeffs: list, cache_dir) -> str:
    """Generate a fast Julia filter script via CSE on the combined Σ c_n H^n expression.

    Unlike :func:`build_julia_filter_from_snippets`, which calls separate
    ``@inline`` functions per H^n per grid point (preventing JIT CSE across
    function boundaries), this function:

    1. Loads raw (unexpanded) ``Pc_n``, ``Ps_n`` from ``H_power_{n}.pkl``.
    2. Forms ``Pc_total = Σ_n c_n * Pc_n``, ``Ps_total = Σ_n c_n * Ps_n``
       without calling ``sp.expand()``.
    3. Applies ``sp.cse([Pc_total, Ps_total])`` to extract shared subexpressions.
    4. Generates Julia with a temp-variable block, ``@inbounds @simd for i in 1:N``,
       and ``Threads.@threads`` for wave-level parallelism.

    This mirrors :func:`build_julia_hn_cse_script` but applied to the full
    filter sum.  Julia sees a single combined expression per grid point, so its
    LLVM backend can register-allocate and vectorise it optimally — recovering
    the ~0.04 s/wave speed achieved by the old monolithic pipeline.

    Parameters
    ----------
    m : int           Chebyshev polynomial order
    coeffs : list     [c_0, c_1, …, c_m] baked as Float64 literals
    cache_dir : Path  directory containing ``H_power_{n}.pkl`` files

    Returns
    -------
    str  Complete Julia source.

    Call convention (no c args, identical to :func:`build_julia_hn_cse_script`)::
    ...
    # ── assemble Pc_total = Σ c_n Pc_n, Ps_total = Σ c_n Ps_n (no expand) ──────
    ...
    for n in range(m + 1):
        ...
        if c_n == 0.0:
            continue
        if n == 0:
            # H^0 · ψ = sin(k·r+b): only sine envelope
            Ps_total = Ps_total + c_n
        else:
            with open(cache_dir / f'H_power_{n}.pkl', 'rb') as fh:
                data = pickle.load(fh)
            if data['Pc'] != 0:
                Pc_total = Pc_total + c_n * data['Pc']
            if data['Ps'] != 0:
                Ps_total = Ps_total + c_n * data['Ps']

    # ── CSE on the combined expression ───────────────────────────────────────────
    repl, (red_cos, red_sin) = sp.cse(
        [Pc_total, Ps_total], symbols=sp.numbered_symbols('_t'))
    ...
    L = ['# Auto-generated by symbolic_code.julia_codegen (CSE filter) — DO NOT EDIT',
         f'# f(H)·ψ = Σ_n c_n H^n·ψ via sp.cse() on the combined raw expression.',
         f'# Polynomial order: m = {m}',
         f'# total_ops={total_ops}  (temps={ops_temps}  reduced={ops_reduced})',
         '#',
         '# Usage: julia <script>.jl grid.bin kvals.bin out.bin N n_waves',
