...
def build_julia_combined_filter_script(Hn_terms_list: list) -> str:
    ...
    Unlike :func:`build_julia_batch_script` (which bakes one combined
    symbolic expression), this function processes each H^n independently
    and passes Chebyshev coefficients as CLI arguments.  The same compiled
    script therefore works for **any** (E_lo, E_hi) — only the coefficients
    change, not the source.

    Construction is fast because ``group_by_exp_combined`` and
    ``apply_horner`` are applied to each compact H^n expression separately,
    never to the huge symbolic sum Σ c_n H^n·ψ.

    Parameters
    ----------
    Hn_terms_list : list of length m+1
        ``Hn_terms_list[n] = (terms_cos_n, terms_sin_n)`` where each
        ``terms_*`` is the output of ``apply_horner(group_by_exp_combined(…))``
        for the cos (Pc_n) and sin (Ps_n) envelopes of H^n·ψ.
    ...
    * ``grid.bin``  – ``[X_flat; Y_flat; Z_flat]`` (3N float64, little-endian)
    * ``kvals.bin`` – ``[kx0,ky0,kz0,b0, kx1,…]``  (4*n_waves float64)
    * ``c0 … cm``   – Chebyshev monomial coefficients as plain float strings
    * ``out.bin``   – result: n_waves×N float64, wave-major order
    * Stdout (last line): JSON ``{warmup_s, eval_s, n_eval_waves}``
    """
    m = len(Hn_terms_list) - 1  # polynomial order
    ...
    # ── per-(n, group) inline functions ──────────────────────────────────────
    # Naming: _poly_c_n{n}_g{g} / _exp_c_n{n}_g{g}  (cos envelope = Pc_n)
    #         _poly_s_n{n}_g{g} / _exp_s_n{n}_g{g}  (sin envelope = Ps_n)
    ...
    # ── eval_one_wave! ────────────────────────────────────────────────────────
    # Vectorised trig (SLEEF SIMD via @.), then scalar accumulation loop.
    # Coefficients are in c[1]..c[m+1] (Julia 1-indexed).
    lines_accum_cos = []
    lines_accum_sin = []
    for n, (terms_cos_n, terms_sin_n) in enumerate(Hn_terms_list):
        cn = f'c[{n+1}]'
        for g in range(len(terms_cos_n)):
            lines_accum_cos.append(
                f'        _sum_cos += {cn}'
                f' * _exp_c_n{n}_g{g}(xi,yi,zi)'
                f' * _poly_c_n{n}_g{g}(xi,yi,zi,kx,ky,kz)')
        for g in range(len(terms_sin_n)):
