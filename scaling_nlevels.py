"""
scaling_nlevels.py
T (wall time) 和 N_H (Hψ 次数) 关于 n_levels 的 scaling。

策略：固定 N=64，target=-0.22，blocksize=1。
n_levels_list = np.arange(1, 21, 2)  → [1, 3, 5, ..., 19]

输出（scaling_results/）
    scaling_nlevels_TIMESTAMP.json
    scaling_nlevels_TIMESTAMP.png
"""
...
# 主循环
# ──────────────────────────────────────────────
print(f"\n开始 scaling（{len(N_LEVELS_LIST)} 个 n_levels，target={TARGET}）...")
results = []

for n_levels in N_LEVELS_LIST:
    ncv = max(80, 2 * int(n_levels))
    print(f"  n_levels={n_levels:2d}  ncv={ncv} ...", end=" ", flush=True)
    t0 = time.perf_counter()
    try:
        evals, _, stats = primme.eigsh(
            H_op, k=int(n_levels), which=TARGET,
            method="PRIMME_JDQMR",
    ...
    row = dict(n_levels=int(n_levels), N=N, target=TARGET, blocksize=BLOCKSIZE,
...
# ──────────────────────────────────────────────
output = {
    "script": "scaling_nlevels.py",
    "datetime": TS,
    "config": dict(N=N, TARGET=TARGET, BLOCKSIZE=BLOCKSIZE, TOL=TOL,
                   N_LEVELS_LIST=N_LEVELS_LIST.tolist()),
    "results": results,
}
json_path = OUT_DIR / f"scaling_nlevels_{TS}.json"
...
for ax, ykey, ylabel in zip(
    ...
    x_vals = np.array([r["n_levels"] for r in rows])
    y_vals = np.array([r[ykey]       for r in rows])
    ax.loglog(x_vals, y_vals, "o", color="steelblue", markersize=8, zorder=3,
              label="measured")

    a, b, r2 = power_fit(x_vals, y_vals)
    if b is not None:
        x_fit = np.logspace(np.log10(x_vals.min()), np.log10(x_vals.max()), 200)
        ax.loglog(x_fit, a * x_fit ** b, "--", color="steelblue", lw=1.8,
                  label=f"∝ n_levels^{b:.2f}  (R²={r2:.3f})")

    ax.set_xlabel("n_levels", fontsize=12)
    ax.set_ylabel(ylabel, fontsize=12)
    ax.set_title(f"{ylabel} vs n_levels\n(N={N}, target={TARGET}, blocksize={BLOCKSIZE})")
    ax.legend(fontsize=9)
    ax.grid(True, which="both", alpha=0.35)

fig.tight_layout()
plot_path = OUT_DIR / f"scaling_nlevels_{TS}.png"
