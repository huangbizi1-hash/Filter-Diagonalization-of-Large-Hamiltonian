...
def run(args):
    ...
    # Bug fix: count_cse_occurrences needs both repl and reduced expressions
    ...
    # Strategies 4, 5, 6: use build_baseline (group_by_exp + Horner + precomputed exp buffers)
    # Bug fix: pass group_by_exp_combined output directly — build_baseline calls apply_horner internally
    # Bug fix: capture the 4-tuple return to get post-optimisation ops_post
    ...
    if args.no_timing:
        ...
        with tempfile.TemporaryDirectory(prefix='hn_methods_') as td:
            for k, (jl, ops) in strategies.items():
                ...
                # Bug fix: warmup_s can be None if Julia JSON parsing fails
